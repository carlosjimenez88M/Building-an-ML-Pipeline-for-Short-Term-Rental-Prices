...
- **GitHub Repository:** [Building an ML Pipeline for Short-Term Rental Prices in NYC](https://github.com/carlosjimenez88M/Building-an-ML-Pipeline-for-Short-Term-Rental-Prices/tree/prod)
- **Weights & Biases Project:** [NYC Airbnb Pipeline Project](https://wandb.ai/danieljimenez88m/nyc_airbnb?nw=nwuserdanieljimenez88m)


### **Project Overview**  
This project implements an end-to-end machine learning pipeline for predicting short-term rental prices in NYC using Weights & Biases (W&B) for experiment tracking and MLflow for model management. The workflow consists of multiple stages, including data ingestion, cleaning, validation, model training, hyperparameter optimization, and performance evaluation. The best model is selected using a **grid search**, systematically evaluating different hyperparameter configurations to identify the most effective setup for minimizing prediction error. The final trained model is logged as an artifact in W&B, ensuring full reproducibility and version control. The full pipeline can be explored in **Weights & Biases**, accessible at:  
🔗 [W&B Project: NYC Airbnb Pricing](https://wandb.ai/danieljimenez88m/nyc_airbnb?nw=nwuserdanieljimenez88m)  
...
1. **Clone the repository:**  
   ```bash
   git clone https://github.com/carlosjimenez88M/Building-an-ML-Pipeline-for-Short-Term-Rental-Prices/tree/prod
   cd carlosjimenez88M/Building-an-ML-Pipeline-for-Short-Term-Rental-Prices
   ```
...
5. **Train the model with optimal hyperparameters:**  
   ```bash
   python src/train_random_forest/run.py --trainval_artifact nyc_airbnb/trainval_data.csv:latest \
                                         --val_size 0.2 \
                                         --random_seed 42 \
                                         --stratify_by neighbourhood_group \
                                         --rf_config configs/rf_best_params.json \
                                         --max_tfidf_features 15 \
                                         --output_artifact trained_random_forest_regression
   ```

6. **Check the trained model in W&B and verify performance on the test set.**  

### **Hyperparameter Optimization Explained**  
Hyperparameter tuning was conducted using a **grid search**, systematically iterating over a predefined set of hyperparameters to identify the configuration that yields the best model performance. This approach ensures that every possible combination of values is evaluated, providing a structured and exhaustive exploration of the hyperparameter space.  

The grid search was applied to the **Random Forest Regressor**, optimizing parameters such as:  
...
Each model configuration was evaluated based on **Mean Absolute Error (MAE)**, ensuring that the selected model minimizes prediction error while generalizing well to unseen data. The best model was **automatically tagged as "prod"** in W&B, making it available for further validation and deployment.
